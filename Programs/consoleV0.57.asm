...
; Variable map
    ...
    ; rv+34 bool equal returned by checkOffset
...
writeChar:      ;void writeChar(char c)
    ...
    LOD Ra, [#rv+34]                ; see value returned by checkOffset (from backspace)
    MOV Rb, #0x01
    CMP Ra, Rb                      ; compare
    JNZ [#.continueNormal-start]    ; if false, continue normally
    MOV Rc, #0x01                   ; else has 1px offset
    MOV Rb, #0x00
    STO [#rv+34], Rb          ; Reset register (false by default)
    MOV PC, #.printC-start
...
checkOffset:         ; int checkOffset
    ...
    MOV Rb, #0x00                           ; false by default
    LDX Si, [#rv+11]                        ; load *endInBuffer
    DEC Si                                  ; decrease pointer
    LOD Ra, [Si]                            ; load last char of inputBuffer
    ...
    DEC Rd                    ; decrease counter
    JNZ [#.loop-start]        ; if no coincidence, exit
    MOV Rb, #0x00             ; no offset

    .exit:
    STO [#rv+34], Rb                        ; store result, Equal=1, Different=0
    POP Di
    POP Si
    POP Rd
    POP Rc
...
addDigit:
    ...
    DEC Di                                  ; Di points to mantissa low
...
processCMD:
    ...
    MOV Rb, #0x01
    CMP Ra, Rb                              ; compare
    JNZ [#.casePOLL-start]                   ; if false, check next command
...
kbHandler:  ; ISR(vector keyboard)		Starts in ram at 0xFEFE
	...
	.pressed:									; input is make code
        ...
        STO [#rv+7], Rb
        CALL [#writeChar-start]
		LOD Rb, [#rv]                           ; load cursor X
        MOV Rd, #0x06
        SUB Rb, Rd                              ; set previous cursor position
        STO [#rv], Rb                           ; and store it

        CALL [#checkOffset-start]               ; check with input buffer (return observed in writeChar)
